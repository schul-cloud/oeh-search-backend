...
from schulcloud.adhoc_tasks.upload_sportinhalt_brandenburg.upload_user_videos_permissions import \
    edusharing_get_permission_groups, edusharing_add_to_permission_groups

import pathlib

# Load information required to connect to Edu-Sharing.
from schulcloud.adhoc_tasks.upload_sportinhalt_brandenburg.utils import get_base_headers
    ...
    for node_id in tqdm(sorted(list(node_id_to_response.keys()), key=lambda x: node_id_to_response[x]["name"]),
        ...
        for k, v in attribute_values.items():
            if k == "ccm:wwwurl":
                ...
                property_value = general_keywords
